...
echo "$($PSQL "SELECT name FROM games AS g INNER JOIN teams AS t ON t.team_id = g.winner_id WHERE year = 2018 AND round = 'Final' AND g.winner_id = t.team_id")"
...
echo "$($PSQL "SELECT year, name FROM teams AS t INNER JOIN games AS g ON t.team_id = g.winner_id WHERE round = 'Final' AND team_id = winner_id ORDER BY name DESC")"
